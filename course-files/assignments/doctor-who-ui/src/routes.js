...
router.route("/doctors")
    .get((req, res) => {
        ...
        Doctor
            .find({})
            .sort('ordering') // sorts by the first season in seasons array
            .then(data => {
...
router.route("/doctors/:id/companions")
    .get((req, res) => {
        Companion
            .find({'doctors': {'$eq': req.params.id} })
            .sort('seasons.0') // sorts by the first season in seasons array
            .then(data => {
                res.status(200).send(data);
            })
            .catch(err => {
...
router.route("/doctors/:id/goodparent")
    .get(async (req, res) => {
        ...
        try {
            ...
            const living = await Companion.find({
                'doctors': {'$eq': req.params.id}
            });
            res.status(200).send(living.length === companions.length);
        } catch(err) {
            res.status(404).send(err);
        }
    });
