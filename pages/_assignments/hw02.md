...
Then, either integrate the new files into your existing `hw02-cs396` folder (#1) or replace your `hw02-cs396` with the files you just downloaded (#2). Pick one of the following:
...
We went over some different ways of formulating MongoDB queries in [Lecture 5](../lectures/lecture05), which may be useful to you (see code samples, including the `samples.js` file and the `answers` folder). There are also some useful links to the MongoDB documentation in Lecture 5, for your reference.
